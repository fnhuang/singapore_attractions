...
class SentimentAnalyzer():
    ...
    def __init__(self, dir, model):
        ...
        if not os.path.exists(f"{self.dir}/training/model"): os.makedirs(f"{self.dir}/training/model")
    ...
    def _remove_most_common_words(self, documents):
        logging.info("Define most common words...")
    ...
    def prepare_documents(self):
        ...
        logging.info("Perform undersampling...")
    ...
    def transform_into_featuresets(self, documents):
        ...
        logging.info("Features length:", len(self.features))
        featuresets = []

        logging.info("Transforming into featuresets....")
        doc_count = 0
        for doc in documents:
    ...
    def train(self, validation_ratio):
        ...
        for cls in self.classes:
            logging.info(f"{cls} accuracy:",
                  (nltk.classify.accuracy(self.classifier, [v for v in validset if v[1] == cls])) * 100)
        logging.info("Classifier accuracy percent:", (nltk.classify.accuracy(self.classifier, validset)) * 100)
    ...
    def classify(self):
        ...
        files = [os.path.basename(x) for x in glob.glob(f"{self.dir}/testing/data/*.csv")]
