...
class TfidfCluster():
    ...
    def get_local_foreigner_difference(self, filter_xtrim):
        ...
        for doc_num in self.id2doc:
            ...
            print("\r",end="")
            print(f"Transforming data into dictionary {int(doc_count/len(self.id2doc)) * 100} percent", end="", flush=True)
        ...
        with open(f"{self.dir}/{self.sentiment}_results/local_v_foreign.txt","w",encoding="utf8") as gwriter:
            ...
            for doc_name in doc_pairs:
                ...
                gwriter.flush()
                print("\r",end="")
                print(f"Calculating cosine sim {int(doc_count/len(doc_pairs)) * 100} percent", end="", flush=True)
        gwriter.close()
